...
trait OutMapper[T] extends ResultSetUtils {
  ...
  def typeOfExpressionToString: String =
    sample.asInstanceOf[Object].getClass.getName
...
class ColumnToTupleMapper(val outMappers: Array[OutMapper[_]]) {
  ...
  def typeOfExpressionToString(idx: Int): String =
  ...
  def mapToTuple[T](rs: ResultSet): T = {
    ...
    val res = size match {
      ...
      case 6 =>
        (m(0).map(rs), m(1).map(rs), m(2).map(rs), m(3).map(rs), m(4).map(rs), m(5).map(rs))
      case 7 =>
        (m(0).map(rs), m(1).map(rs), m(2).map(rs), m(3).map(rs), m(4).map(rs), m(5).map(rs), m(6).map(rs))
      case 8 =>
...
//  }
  ...
  def isNoneInOuterJoin(rs: ResultSet): Boolean = {
    ...
    //outMappers
    for (col2TupleMapper <- List(groupKeysMapper, groupMeasuresMapper)
           .filter(_.isDefined)
           .map(_.get);
