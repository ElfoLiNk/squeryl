...
trait DatabaseAdapter {
  ...
  def databaseTypeFor(fmd: FieldMetaData): String =
    fmd.explicitDbTypeDeclaration.getOrElse(
      fmd.schema
        .columnTypeFor(fmd, fmd.parentMetaData.viewOrTable.asInstanceOf[Table[_]])
        .getOrElse {
  ...
  def executeUpdateForInsert(s: AbstractSession, sw: StatementWriter, ps: PreparedStatement): Int = exec(s, sw) {
    params =>
...
//  def writeUniquenessConstraint(columnDefs: Seq[FieldMetaData], nameOfCompositeKey: Option[String]) = ""
  ...
  def quoteName(s: String): String =
    s.split('.').map(quoteIdentifier).mkString(".")
