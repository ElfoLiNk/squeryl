...
object FieldReferenceLinker {
  ...
  def inspectedQueryExpressionNode: QueryExpressionNode[_] =
    _yieldInspectionTL.get.queryExpressionNode

  private val _yieldValues = new ThreadLocal[ArrayBuffer[AnyRef]]

  private val __lastAccessedFieldReference =
  ...
  private val _compositeKeyMembers =
  ...
  private class PosoPropertyAccessInterceptor(val viewExpressionNode: ViewExpressionNode[_]) extends MethodInterceptor {
    ...
    private def _intercept(
      ...
      if (isComposite) {
        ...
        ck._members = Some(
          _compositeKeyMembers.get.get
            .map(new SelectElementReference[Any, Any](_, NoOpOutMapper))
        )
      ...
      if (fmd.isDefined) {
        ...
        if (_compositeKeyMembers.get == null) {
          ...
          _compositeKeyMembers.get.get
            .append(viewExpressionNode.getOrCreateSelectElement(fmd.get))
      }
