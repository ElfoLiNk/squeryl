...
class Schema(implicit val fieldMapper: FieldMapper) {
  ...
  private val _manyToManyRelations =
    new ArrayBuffer[ManyToManyRelation[_, _, _]]

  private val _columnGroupAttributeAssignments =
    new ArrayBuffer[ColumnGroupAttributeAssignment]

  private[squeryl] val _namingScope = new mutable.HashSet[String]

  private[squeryl] def _addRelation(r: OneToManyRelation[_, _]): Unit =
    _oneToManyRelations.append(r)

  private[squeryl] def _addRelation(r: ManyToManyRelation[_, _, _]): Unit =
  ...
  def findAllTablesFor[A](c: Class[A]): Traversable[Table[_]] =
    _tables
      ...
      .asInstanceOf[Traversable[Table[_]]]
  ...
  private def _writeIndexDeclarationIfApplicable(
    ...
    (unique, indexed) match {
      case (None, None) => None
      case (Some(_), None) =>
        Some(_dbAdapter.writeIndexDeclaration(cols, None, name, isUnique = true))
      case (None, Some(Indexed(idxName))) =>
        Some(
          _dbAdapter
            .writeIndexDeclaration(cols, idxName, name, isUnique = false)
        )
      case (Some(_), Some(Indexed(idxName))) =>
        Some(
          _dbAdapter
            .writeIndexDeclaration(cols, idxName, name, isUnique = true)
        )
    }
  }
  ...
  protected def table[T](
    name: String,
    prefix: String
  )(implicit manifestT: Manifest[T], ked: OptionalKeyedEntityDef[T, _]): Table[T] = {
    val typeT = manifestT.runtimeClass.asInstanceOf[Class[T]]
  ...
  private[squeryl] def _addTable(t: Table[_]): Unit =
    _tables.append(t)
...
//      )
  ...
  def defaultColumnAttributesForKeyedEntityId(
    typeOfIdField: Class[_]
  ): Set[_ >: PrimaryKey <: AttributeValidOnNumericalColumn with Product with Serializable] =
    if (typeOfIdField.isAssignableFrom(classOf[java.lang.Long]) || typeOfIdField
  ...
  protected def autoIncremented(sequenceName: String) =
    AutoIncremented(Some(sequenceName))
  ...
  def columns(fieldList: TypedExpression[_, _]*) =
    new ColGroupDeclaration(fieldList.map(_._fieldMetaData))
