...
trait TypedExpression[A1, T1] extends ExpressionNode {
  ...
  def isNotNull =
    new PostfixOperatorNode("is not null", this) with LogicalBoolean
  ...
  def regex(pattern: String) =
    new FunctionNode(pattern, Seq(this)) with LogicalBoolean {

      override def doWrite(sw: StatementWriter): Unit =
        Session.currentSession.databaseAdapter
          .writeRegexExpression(outer, pattern, sw)
    }
...
trait FloatTypedExpressionFactory[A1, T1] extends TypedExpressionFactory[A1, T1] with IdentityFloatifier[A1, T1] {
  ...
  def floatify(v: ExpressionNode): TypedExpressionConversion[A1, T1] =
    convert(v)
}
...
trait JdbcMapper[P, A] {
  ...
  def map(rs: ResultSet, i: Int): A =
    convertFromJdbc(extractNativeJdbcValue(rs, i))
}
...
abstract class NonPrimitiveJdbcMapper[P, A, T](
  ...
  def createFromNativeJdbcValue(v: P): TypedExpression[A, T] =
    create(convertFromJdbc(v))
...
trait TypedExpressionFactory[A, T] {
  ...
  def thisAnyRefMapper: JdbcMapper[AnyRef, A] =
    this.asInstanceOf[JdbcMapper[AnyRef, A]]
