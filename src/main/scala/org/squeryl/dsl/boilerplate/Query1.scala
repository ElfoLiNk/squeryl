...
class Query1[T1, R](t1: Queryable[T1], f: (T1) => QueryYield[R], isRoot: Boolean, unions: List[(String, Query[R])])
...
class Query2[T1, T2, R](
  ...
  val ast: QueryExpressionNode[R] =
    buildAst(f(sq1.sample, sq2.sample), sq1, sq2)
}
...
class Query3[T1, T2, T3, R](
  ...
  val ast: QueryExpressionNode[R] =
    buildAst(f(sq1.sample, sq2.sample, sq3.sample), sq1, sq2, sq3)
}
