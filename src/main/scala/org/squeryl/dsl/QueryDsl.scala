...
//      queryToIterable(view2QueryAll(t))
  ...
  def not(b: LogicalBoolean) =
    new FunctionNode("not", Seq(b)) with LogicalBoolean
  ...
  implicit val numericComparisonEvidence: CanCompare[TNumeric, TNumeric] =
    new CanCompare[TNumeric, TNumeric]
  implicit val dateComparisonEvidence: CanCompare[TOptionDate, TOptionDate] =
    new CanCompare[TOptionDate, TOptionDate]
  ...
  implicit val uuidComparisonEvidence: CanCompare[TOptionUUID, TOptionUUID] =
    new CanCompare[TOptionUUID, TOptionUUID]
  ...
  class ManyToManyRelationBuilder[L, R](
    ...
    )(implicit manifestA: Manifest[A], schema: Schema, kedA: KeyedEntityDef[A, _]): ManyToManyRelationImpl[L, R, A] = {
  ...
  class ManyToManyRelationImpl[L, R, A](
    ...
    private val (_leftEqualityExpr, _rightEqualityExpr) = {
      ...
      if (e2_._1
            .filterDescendantsOfType[ConstantTypedExpression[_, _]]
            .nonEmpty)
    ...
    private val (leftPkFmd, leftFkFmd) =
      _splitEquality(_leftEqualityExpr, thisTable, isSelfReference = false)

    private val (rightPkFmd, rightFkFmd) =
    ...
    private def _associate[T](o: T, m2m: ManyToMany[T, A]): A = {
      ...
      try {
        ...
        case e: SQLException =>
          if (Session.currentSession.databaseAdapter
                .isNotNullConstraintViolation(e))
      ...
      new DelegateQuery(q) with ManyToMany[R, A] {
        ...
        def _whereClauseForAssociations(a0: A): LogicalBoolean = {
          ...
          FieldReferenceLinker
            .createEqualityExpressionWithLastAccessedFieldReferenceAndConstant(leftPk, None)
        }
        ...
        def _equalityForRightSide(a0: A, r: R): LogicalBoolean = {
          ...
          FieldReferenceLinker
            .createEqualityExpressionWithLastAccessedFieldReferenceAndConstant(rightPk, None)
        }
