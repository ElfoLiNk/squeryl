...
class TestInstance(schema: SchoolDb) {
  ...
  val counterpoint =
    courses.insert(new Course("Counterpoint", feb2010, None, 0, None, true))
  val mandarin =
    courses.insert(new Course("Mandarin 101", feb2010, None, 0, None, true))
...
//  }
  ...
  test("DeepNest2") {
    ...
    val q =
      from(from(from(professors)(p0 => select(p0)))(p1 => select(p1)))(p2 => select(p2))
  ...
  test("OptionAndNonOptionMixInComputeTuple") {
    val _: Product4[Option[Float], Option[Float], Option[Double], Long] =
      avgStudentAgeFunky
...
//    courses.update(ht)
  ...
  test("PartialUpdateWithSubQueryInSetClause") {
    ...
    val expected: Float = from(professors)(
      p0 => where(tournesol.id === p0.id or p0.id === zarnitsyn.id) compute (nvl(avg(p0.yearlySalary), 123))
    )

    update(professors)(
      p =>
        where(p.id === tournesol.id)
          set (p.yearlySalary := from(professors)(
            p0 => where(p.id === p0.id or p0.id === zarnitsyn.id) compute (nvl(avg(p0.yearlySalary), 123))
          ))
    )
  ...
  test("AvgBigDecimal") {
    ...
    val expectedAvgWeight_ =
      professors.allRows.map(_.weightInBD).filter(_ != None).map(_.get)
