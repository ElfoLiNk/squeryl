...
abstract class LogicalBooleanObjTests extends SchemaTester with RunTestsInsideTransaction {
  ...
  test("and/or operators for Option[LogicalBoolean]") {
    ...
    def q1(opt: Option[Int]) =
      from(dummy)(d => where(TrueLogicalBoolean and opt.map(_ === d.p2)) select d)
    q1(none).size should equal(2)
    q1(some).size should equal(1)

    def q2(opt: Option[Int]) =
      from(dummy)(d => where(FalseLogicalBoolean or opt.map(_ === d.p2)) select d)
    q2(none).size should equal(0)
    q2(some).size should equal(1)

    def q3(opt: Option[Int]) =
      from(dummy)(d => where(FalseLogicalBoolean and opt.map(_ === d.p2)) select d)
