...
//    testKeyedEntityImplicitLookup
  ...
  test("UpperAndLowerFuncs") {
    try {
      ...
      val expected =
        List(mongoSantaMaria.firstName, ponchoSanchez.firstName).map(s => s.toUpperCase + s.toLowerCase)
  ...
  test("ConcatFunc") {
    ...
    val expected =
      List(mongoSantaMaria.firstName, ponchoSanchez.firstName).map(s => s + "zozo")
  ...
  ignore("TimestampPartialUpdate") {
    ...
    val res = artists
      .where(_.firstName === mongoSantaMaria.firstName)
      .single
      .timeOfLastUpdate
  ...
  test("TimestampDownToMillis") {
    ...
    if (!dbAdapter.isInstanceOf[MSSQLServer] && !dbAdapter
      ...
      val shouldBeSome = artists
        .where(a => a.firstName === mongoSantaMaria.firstName and a.timeOfLastUpdate === tX2)
        .headOption
  ...
  test("validateScalarQueryConversion1") {
    ...
    val d = avgSongCountForAllArtists
    //println("d=" + d)
  ...
  test("Inhibit both sides of LogicalBoolean") {
    from(artists)(
      a =>
        where(
          (a.age === 1000000)
            .inhibitWhen(true) and (a.id between (999, 1000)).inhibitWhen(true)
        )
          select a
