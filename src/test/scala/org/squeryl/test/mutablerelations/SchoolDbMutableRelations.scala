...
class Professor(val lastName: String) extends SchoolDb2Object {
  ...
  lazy val courses: StatefulManyToMany[Course, CourseAssignment] =
    SchoolDb2.courseAssignments.leftStateful(this)
}
...
class Course(val subjectId: Long) extends SchoolDb2Object {
  ...
  lazy val professors: StatefulManyToMany[Professor, CourseAssignment] =
    SchoolDb2.courseAssignments.rightStateful(this)

  lazy val students: StatefulManyToMany[Student, CourseSubscription] =
    SchoolDb2.courseSubscriptions.leftStateful(this)

  lazy val subject: StatefulManyToOne[Subject] =
    SchoolDb2.subjectToCourses.rightStateful(this)
}

class Student(val firstName: String, val lastName: String) extends SchoolDb2Object {

  lazy val courses: StatefulManyToMany[Course, CourseSubscription] =
    SchoolDb2.courseSubscriptions.rightStateful(this)
}

class Subject(val name: String) extends SchoolDb2Object {

  lazy val courses: StatefulOneToMany[Course] =
    SchoolDb2.subjectToCourses.leftStateful(this)
}
...
abstract class SchoolDb2MetableRelations extends SchemaTester with QueryTester with RunTestsInsideTransaction {
  ...
  def instance = new {
    ...
    val professeurTournesol: Professor =
      professors.insert(new Professor("Tournesol"))
    val madProfessor: Professor =
      professors.insert(new Professor("Mad Professor"))
    ...
    val computationTheory: Subject =
      subjects.insert(new Subject("Computation Theory"))
